...
public class KeyCombinator 
{
	...
	public void combinator(int Smax)
	{
		
		//Initiate list of KeyGroup containing only 1 key in each group
		for(int i=0;i<Keys.size();i++)
		{
			KeyGroup KG=new KeyGroup((Key)Keys.elementAt(i));
		...
		//Generate combinations of width i <= Smax on every iteration
				for(int i=2;i<=Smax;i++)
				{
					...
					for(int j=FROM,l=1;j<TO;j++,l++)//for all NEW KeyGroups
					{
						...
						{
							KeyGroup KG=new KeyGroup((KeyGroup)ListofGroups.get(j));//add Key(k) to the current group
							if(KG.addKey((Key)(Keys.elementAt(k)), i)==0)//if new Key was added to the group
							{
		...
		for(int i=0;i<ListofGroups.size();i++)
			...
			System.out.println(((KeyGroup)ListofGroups.elementAt(i)).getValue());
		int sum=0;
		for(int i=1;i<=Smax;i++)
			sum+=Math.nCr(Nkeys, Smax);
		System.out.println("Expected #KeyGroups="+sum);
		System.out.println("Actual #KeyGroups:"+ListofGroups.size());
		/*// OLD LOGIC
	...
	public boolean repeatedKG(KeyGroup KG)
	{
		...
		String str=KG.getValue();
		for(int i=0;i<ListofGroups.size();i++)
			if(str.equalsIgnoreCase((ListofGroups.get(i)).getValue()))
				return true;
			//else
				//System.out.println(str+" != "+(ListofGroups.get(i)).getValue());
		return false;
