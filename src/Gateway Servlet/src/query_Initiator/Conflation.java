...
/*
 * Conflation based on Porter's Stemming Algorithm 
...
public class Conflation	{
    ...
    /** The HashSet representation of the stop word list. It takes, on average,
     * <I>O(1)</I> to search a hash table, as opposed to O(log n) to binary search
     * an array.
     */

	private HashSet<String> sw;
	private String[]	stopWords = {	// 318 [0-317]
					"a",
					"about",
					"above",
					"across",
...
//			if (! isStopWord(0, stopWords.length-1, strng)) {
	...
	public static void main (String[] args) throws IOException {
		...
		System.out.println(w.applyConflation(strng));
	...
	public Conflation()
	{
		...
		sw = new HashSet<String>(stopWords.length);
	...
	public String applyConflation(String input)
	{
		...
		for(int j=0;j<words.length;j++)
		{
			...
			words[j]=stripAffixes(words[j]);
			
			//System.out.println("After: "+words[j]+".");
			
			if(words[j].isEmpty()==false&&words[j]!=" ")
			{
				if(j==0)
					input=words[j];
				else
